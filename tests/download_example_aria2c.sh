...
# Check if aria2c is available, fallback to curl if not
if command -v aria2c >/dev/null 2>&1; then
    # Use aria2c with robust download settings
    aria2c -q -x 4 -s 4 -c --max-tries=5 --retry-wait=3 --dir="$test_data_dir" --out="$file_name" "$url"
else
